...
import { Search, User, Bell, Wallet, ChevronDown, LogOut, ShieldCheck, ClipboardList, Menu, X, Gavel, PlusCircle, Home, Package, ArrowRight, Instagram, Facebook, Twitter, Mail, LockKeyhole, Award, Gem, ChevronRight } from 'lucide-react';
...
const Layout = () => {
    ...
    useEffect(() => {
        if (localStorage.getItem('token')) {
            ...
            client.get('/auth/me/notifications')
        ...
        ws.onmessage = (event) => {
            try {
                ...
                setNotifications(prev => {
                    if (incoming.id && prev.some(n => n.id === incoming.id)) {
                        return prev;
                    }
    ...
    const handleGoHome = () => {
        ...
        if (location.pathname === '/') {
            ...
            navigate('/');
            // ScrollToTop in App.jsx will handle scroll
        }
    };
        ...
        <div className="min-h-screen flex flex-col bg-[#F8F1E6] text-[#2F2418]">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="hidden xl:flex items-center bg-white/[0.04] px-4 py-2.5 w-56 border border-[#2F2418]/10 focus-within:border-[#9A6A2F]/60 focus-within:bg-white/[0.07] transition-all">
                            ...
                            <input 
                                ...
                                className="bg-transparent border-none outline-none text-sm w-full text-[#2F2418] placeholder-[#2F2418]/35"
                            />
                        </div>
                                            ...
                                            <ul className="max-h-72 overflow-y-auto scrollbar-thin divide-y divide-slate-50">
                                                {notifications.length > 0 ? notifications.map(n => (
                                                    <li key={n.id} className={`p-4 hover:bg-white/5 transition-colors relative ${n.is_read ? 'opacity-60' : ''}`}>
                                                        ...
                                                        <Link 
                                                            to={
                                                                n.type === 'auction_won' ? `/invoices/${n.reference_id}/checkout` : 
                                                                n.type === 'auction_failed' ? `/auctions/create?relist_id=${n.reference_id}` :
                                                                "/profile"
                                                            } 
                                                            onClick={() => {
                                                                if (!n.is_read) {
                                                                    client.post(`/auth/notifications/${n.id}/read`)
                                                                        .then(() => {
                        ...
                        <button 
                            ...
                            className="lg:hidden p-2 text-[#2F2418]/72 hover:text-[#9A6A2F] hover:bg-white/5 transition-colors"
                            aria-label="Menu"
                        >
            ...
            <footer className="relative overflow-hidden bg-gradient-to-br from-[#E8D5B7] via-[#EFE2CF] to-[#F5E6D3] text-[#2F2418] border-t-2 border-[#9A6A2F]/25 mt-32">
                ...
                <div className="absolute inset-0 opacity-[0.15]" style={{ 
                    backgroundImage: `
                        radial-gradient(circle at 20% 20%, rgba(154,106,47,0.25), transparent 40%),
                        radial-gradient(circle at 80% 80%, rgba(154,106,47,0.15), transparent 40%),
                        radial-gradient(circle at 40% 60%, rgba(245,230,200,0.2), transparent 35%)
                    ` 
                }} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-y-12 gap-x-8 xl:gap-x-16">
                        ...
                        <div className="md:col-span-2 xl:col-span-4 xl:pr-6">
                            <div className="flex items-center gap-3.5 mb-6">
                                <div className="w-12 h-12 border-2 border-[#9A6A2F]/70 bg-[#FFF8ED] flex items-center justify-center shadow-[0_4px_16px_rgba(154,106,47,0.15)]">
                            ...
                            <div className="flex items-center gap-3">
                                {[Instagram, Facebook, Twitter].map((Icon, idx) => (
                                    <a 
                                        key={idx} 
                                        href="/" 
                                        className="w-11 h-11 border-2 border-[#9A6A2F]/30 bg-[#FFF8ED]/50 flex items-center justify-center text-[#2F2418]/60 hover:text-[#9A6A2F] hover:border-[#9A6A2F] hover:bg-[#FFF8ED] hover:shadow-[0_4px_16px_rgba(154,106,47,0.2)] transition-all duration-300" 
                        ...
                        <div className="min-w-0 xl:col-span-2">
                            ...
                            <ul className="space-y-3.5">
                                <li>
                                    <button type="button" onClick={handleGoAuctionFloor} className="text-[14px] text-[#2F2418]/65 hover:text-[#9A6A2F] hover:translate-x-1 transition-all duration-200 inline-flex items-center gap-2 group">
                                        <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                                        Catalogue hiện tại
                                    </button>
                                </li>
                                <li>
                                    <Link to="/auctions/create" className="text-[14px] text-[#2F2418]/65 hover:text-[#9A6A2F] hover:translate-x-1 transition-all duration-200 inline-flex items-center gap-2 group">
                                        <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                                        Gửi hiện vật thẩm định
                                    </Link>
                                </li>
                                <li>
                                    <Link to="/" className="text-[14px] text-[#2F2418]/65 hover:text-[#9A6A2F] hover:translate-x-1 transition-all duration-200 inline-flex items-center gap-2 group">
                                        <ChevronRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                                        Lịch phiên sắp tới
                                    </Link>
                                </li>
                        ...
                        <div className="md:col-span-2 xl:col-span-4 xl:pl-6">
                            ...
                            <p className="text-[14px] text-[#2F2418]/65 leading-relaxed mb-6">
                                Nhận catalogue tuyển chọn và lời mời xem trước các phiên đấu giá kín.
                            </p>
                            <div className="flex border-2 border-[#9A6A2F]/30 bg-[#FFF8ED]/60 shadow-[0_4px_16px_rgba(154,106,47,0.08)] overflow-hidden">
                                <div className="flex items-center pl-4 text-[#9A6A2F]/70">
                                    <Mail className="w-[18px] h-[18px]" />
                                </div>
                                <input 
                                    className="min-w-0 flex-1 bg-transparent px-3.5 py-3.5 text-sm text-[#2F2418] placeholder-[#2F2418]/40 outline-none font-medium" 
                                    placeholder="Email của bạn" 
                                />
                                <button 
                                    className="px-5 text-[#F8F1E6] bg-[#9A6A2F] hover:bg-[#2F2418] transition-all duration-300 hover:shadow-[inset_0_0_20px_rgba(0,0,0,0.1)]" 
                            ...
                            <div className="mt-6 grid grid-cols-3 gap-4">
                                <div className="flex flex-col items-center text-center">
                                    <div className="w-10 h-10 bg-[#9A6A2F]/10 border border-[#9A6A2F]/30 flex items-center justify-center mb-2">
                                        <LockKeyhole className="w-4 h-4 text-[#9A6A2F]" />
                                    </div>
                                    <span className="text-[10px] uppercase tracking-[0.12em] text-[#2F2418]/60 font-semibold">Escrow</span>
                                </div>
                                <div className="flex flex-col items-center text-center">
                                    <div className="w-10 h-10 bg-[#9A6A2F]/10 border border-[#9A6A2F]/30 flex items-center justify-center mb-2">
                                        <Award className="w-4 h-4 text-[#9A6A2F]" />
                                    </div>
                                    <span className="text-[10px] uppercase tracking-[0.12em] text-[#2F2418]/60 font-semibold">Verified</span>
                                </div>
                                <div className="flex flex-col items-center text-center">
                                    <div className="w-10 h-10 bg-[#9A6A2F]/10 border border-[#9A6A2F]/30 flex items-center justify-center mb-2">
                                        <Gem className="w-4 h-4 text-[#9A6A2F]" />
                                    </div>
                                    <span className="text-[10px] uppercase tracking-[0.12em] text-[#2F2418]/60 font-semibold">Curated</span>
                                </div>
                            </div>
                        </div>
                    </div>
